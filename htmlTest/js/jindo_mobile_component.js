/** Jindo Mobile Component(minify:0) : ../www/docs/jindo-mobile/archive/1.8.0/
 *	jindo.m
 *	jindo.m.Component
 *	jindo.m.UIComponent
 *	jindo.m.Animation
...
// 
        ...
        **/
        ...
        hasClickBug : function(){
            ...
            return ( _htOsInfo.iphone || _htOsInfo.ipad || (_htOsInfo.mwin && ((_htOsInfo.version *1) >= 8)) );
        }
    };
...
jindo.m.Animation = jindo.$Class({
	...
	/**
      ...
      @param {Object} [htOption] 초기화 옵션 객체
				@param {String} [htOption.sTargetValue="x"] 애니메이션 동작의 기준이 되는 좌표 (x 또는 y)
				@param {Boolean} [htOption.bUseCircular=false] 순환여부
  ...
  **/	
	...
	_initVar: function(el) {
		...
		this._bUseH = this.option("sTargetValue") === "x" ? true : false;
		// set 이후 설정되는 값.
		this._welTarget = null;	// 하위에서 필수 설정.
		this._nRange = null;	// 패널이 있는 경우, 하위에서 필수 설정.
	},

	/**
	 * [필수 구현]
	 * 대상 컴포넌트를 초기화
	 * @param {HashTable} htElement 
	 */
	set : function(htElement) {}, // 하위에서 필수 구현.

	/**
	 * [필수 구현]
	 * 화면 사이즈를 변경할 경우, 각 패널의 정보를 변경함
	 * @param {HashTable} htElement
	 */
	resize : function(htElement) {
		var aP = htElement["aPanel"];
		var nSizeKey = this._bUseH ? "width" : "height",
			nViewSize = htElement["view"][nSizeKey]();
		return {
			viewSize : nViewSize,
			panelCount : aP.length,
			property : nSizeKey
		};
	}, // 하위에서 구현.
	 ...
	 */
	move : function(nX, nY, option) {},  // 하위에서 필수 구현.
  ...
  **/
	_transitionTime: function (nDuration) {
		nDuration += 'ms';
		this.getTarget(true).css(this._sCssPrefix + "TransitionDuration", nDuration);
  ...
  **/
	_onTransitionEnd : function(we) {
		...
		if(typeof this._htParam.no !== "undefined") {
			this._firePanelEndEvent();
		}
		// 버그 fix. timeout을 주지 않으면 transitionEnd가 죽~ 발생해버림 
		var self = this;
		setTimeout(function() {
			self._animate();
		},0);
	...
	clearDuration : function() {
		this.getTarget(true).css(this.prefix("TransitionDuration"), "");
	},
	...
	_detachTransitionEnd : function() {
		jindo.m.detachTransitionEnd(this.getTarget(), this._wfTransitionEnd);
	},

	_attachTransitionEnd : function() {
		jindo.m.attachTransitionEnd(this.getTarget(), this._wfTransitionEnd);
	},

	_getCurrentPanel : function(nPos) {
		return parseInt(-nPos/this.getRange(),10);
	},

	_getStepCount : function(nStart, nEnd) {
		var nRange = this.getRange(),
			nStartIdx = this._getCurrentPanel(nStart),
			nEndIdx = this._getCurrentPanel(nEnd),
			bNext = nStart > nEnd,
			nCount = Math.abs(nStartIdx - nEndIdx),
			nStartRangePos = nStartIdx * -nRange,
			nEndRangePos = nEndIdx * -nRange;
			
			if(nStart > 0) {
				if( nStart > nStartRangePos && bNext) {
					nCount++;
				}
				if(nEnd != nEndRangePos) {
					nCount++;
				}
			} else if (nStart <= 0) {
				// 앞으로 가면서 마지막판의 경계가 아닌경우
				if(bNext && (nEnd !== nEndRangePos) 
					// 뒤로 가면서 시작판의 경계가 아닌경우
					|| !bNext && (nStart !== nStartRangePos)) {
					nCount++;
				}	
			}
			// console.log("_getStepCount",nStart + "[" + nStartIdx + "]," + nEnd + "[" + nEndIdx + "]," + bNext +" (" + nCount + ")");
		return nCount;
	},

	/**
	 * 애니메이션을 실행한다.
	 * @param  {[Number]} nStartX    [시작 위치]
	 * @param  {[Number]} nStartY    [시작 위치]
	 * @param  {[Number]} nEndX      [종료 위치]
	 * @param  {[Number]} nEndY      [종료 위치]
	 * @param  {[Number]} nDuration [총 이동 시간]
	 * @param  {[HashTable]} htOption [옵션 restore, direct, rearrange]
	 * @method run
	 */
	run: function (nStartX, nStartY, nEndX, nEndY, nDuration, htOption) {
		this.stop();
		htOption = htOption || {};
		...
		var nRange=this.getRange();
		// console.debug("run : (" + nStartX + "," + nStartY + ") -> (" + nEndX + "," + nEndY + ")");

		// panel
		if(nRange) {
			this._processForPanel(nRange, nStartX, nStartY, nEndX, nEndY, nDuration, htOption);
		} else {
			this._aQueue.push(this._makeData(nStartX, nStartY, nEndX, nEndY, nDuration, htOption));
		}
	  /**
	      start 애니메이션이 run되었을때 발생하는 이벤트

	      @event start
	      @param {String} sType 커스텀 이벤트명
	      @param {Number} start 처음 이동하는 좌표
	      @param {Number} end 최종 이동할 좌표
	      @param {Number} next 다음으로 이동하는 경우 true, 이전으로 이동하는 경우 false 
	      @param {Number} totalNo 패널의 개수
	      @param {jindo.$Event} oEvent start 이벤트 객체
	  **/		
		this.fireEvent("start", this._makeData(nStartX, nStartY, nEndX, nEndY, nDuration, htOption));
		this._animate();
	},

	_processForPanel : function(nRange, nStartX, nStartY, nEndX, nEndY, nDuration, htOption) {
		var start, end, startKey, endKey, bNext;
		if(this._bUseH) {
			start = nStartX;
			startKey = "startX";
			end = nEndX;
		...
		var nEaseIn = 0,
			...
			nNextDuration = 0,
			htData = null,
			nCurrentPanel = this._getCurrentPanel(start),
			nStepCount = this._getStepCount(start,end);
		
		// 옵션 데이터 초기화
		this._initOption(htOption, nStepCount,bNext);
		
		// 애니메이션 queue 쌓기
		for(var i=0; i<nStepCount; i++) {
			nEaseIn = 1 - Math.sin(Math.acos((i+1)/nStepCount));
			nNextDuration = nDuration * nEaseIn - nPrevDuration;
			htData = {
				duration: nNextDuration,
				next : bNext
			};
			end = this._getPanelEndPos(start, nCurrentPanel, nRange,bNext);
			htData[startKey] = start;
			htData[endKey] = end;
			htData.no = -end/nRange;

			// direct인 경우 마지막 액션만 추가함.
			if(!htOption.direct || i === nStepCount-1 ) {
				// console.warn("  . (" + htData[startKey] + " -> " + htData[endKey] + ")[" + (htData.no) + "]",
				//  	"range/" + nRange,
				//  	nNextDuration + "ms",
				//  	"next/" + bNext
				// );
				this._aQueue.push(this._makeData(nStartX, nStartY, nEndX, nEndY, nDuration, htOption, htData));
			}
			start = end;
			nPrevDuration += nNextDuration;
		}				
	},

	_initOption : function(htOption, nStepCount, bNext) {
		htOption = htOption || {};
		htOption.next = bNext;
		htOption.restore = htOption.restore || false;
		if(htOption.restore) {
			htOption.moveCount = 0;
		} else {
			htOption.moveCount = htOption.direct ? nStepCount : 1;
		}
		htOption.corrupt = htOption.moveCount > 1 || htOption.rearrange;
	},

	_getPanelEndPos : function(nStart, nCurrentPanel, nRange, bNext) {
		var nPos = 0, 
			bAdjust = (nStart%nRange === 0);
		// console.error(nStart, nCurrentPanel, bNext);
		if(bAdjust) {
			nPos = nStart + (bNext? -nRange : nRange);
		} else {
			if(nStart > 0) {
				nCurrentPanel--;
			}
			nPos = (bNext ? nCurrentPanel+1 : nCurrentPanel) * -nRange;
		}
		return nPos;
	},

	_makeData : function(nStartX, nStartY, nEndX, nEndY, nDuration, htOption, ht) {
		ht = ht || {};
		if(typeof ht.startX === "undefined") { ht.startX = nStartX; }
		if(typeof ht.startY === "undefined") { ht.startY = nStartY; }
		if(typeof ht.endX === "undefined") { ht.endX = nEndX; }
		if(typeof ht.endY === "undefined") { ht.endY = nEndY; }
		if(typeof ht.duration === "undefined") { ht.duration = nDuration; }
		for(var k in htOption) {
			if(htOption.hasOwnProperty(k)) {
				ht[k] = htOption[k];
			}
		}
		return ht;
  ...
  **/
	_animate : function() {
		...
		if(!this._aQueue.length) {
		  /**
		      start 애니메이션이 run이 종료 되었을때 발생하는 이벤트
		      @event end
		      @param {String} sType 커스텀 이벤트명
		      @param {jindo.$Event} oEvent end 이벤트 객체
		  **/	
		  this.fireEvent("end");
			return;
		}
		var self = this, 
			oStep = this._aQueue.shift();
		// console.warn("step",oStep);
		if(!oStep || ( oStep.startX === oStep.endX && oStep.startY === oStep.endY) ) {
			return;
		}
		this._isPlaying = true;
		if(typeof oStep.no !== "undefined") {
			if(!this._firePanelStartEvent(oStep)) {
				this.stop();
				return;
			}
		}
		if(oStep.duration == 0) {
			...
			this._fireMoveEvent(oStep);
			this._isPlaying = false;
			if(typeof oStep.no !== "undefined") {
				self._firePanelEndEvent();
			}
			this._animate();
		} else {
			// Transition을 이용한 animation
			if(this.option("bUseTimingFunction")) {
				// this.move(oStep.startX, oStep.startY, oStep.next);
				this._transitionTime(oStep.duration);
				this._fireMoveEvent(oStep);
				// this._isPlaying = false;
				...
				var startTime = (new Date()).getTime(),
					nStartX = oStep.startX, 
					nStartY = oStep.startY, 
					nEndX = oStep.endX,
					nEndY = oStep.endY,
					nDisX = oStep.endX - nStartX,
					nDisY = oStep.endY - nStartY,
					nDuration = oStep.duration,
					now, nEaseOut;
				(function animate () {
					...
					if (now >= startTime + nDuration) {
						oStep.endX = nEndX;
						oStep.endY = nEndY;
						self._fireMoveEvent(oStep);
						self._isPlaying = false; // 애니메이터를 돌리기위해 설정을 변경함
						if(typeof oStep.no !== "undefined") {
							self._firePanelEndEvent();
						}
						self._animate();
					...
					oStep.endX = nDisX * nEaseOut + nStartX;
					oStep.endY = nDisY * nEaseOut + nStartY;
					self._fireMoveEvent(oStep);
					if (self._isPlaying) {
  ...
  /**
      move timer로 동작할때, 좌표 이동시 발생하는 이벤트 

      @event move
      @param {String} sType 커스텀 이벤트명
      @param {Number} start 처음 이동하는 좌표
      @param {Number} end 이동 진행이 완료된 좌표
      @param {Number} next 다음으로 이동하는 경우 true, 이전으로 이동하는 경우 false 
      @param {jindo.$Event} oEvent move 이벤트 객체
  **/
	_fireMoveEvent : function(oStep) {
		this.move(oStep.endX, oStep.endY, oStep);
		this.fireEvent("move", oStep);
	},

  /**
      panelStart 패널 한판을 온전히 이동할때, 패널 처음좌표에서 발생하는 이벤트
      ...
      @param {Number} end 이동 진행이 완료된 좌표
      @param {Number} next 다음으로 이동하는 경우 true, 이전으로 이동하는 경우 false 
      @param {Number} no 현재 이동하는 패널의 인덱스 (0부터 시작) 
      @param {jindo.$Event} oEvent panelStart 이벤트 객체
  **/
	_firePanelStartEvent : function(oStep) {
			this._htParam.no = oStep.no;
			this._htParam.next = oStep.next;
			this._htParam.restore = oStep.restore;
			this._htParam.moveCount = oStep.moveCount;
			this._htParam.corrupt = oStep.corrupt;
			return this.fireEvent("panelStart", oStep);
	},

  /**
      panelEnd 패널 한판을 온전히 이동할때, 패널 마지막 좌표에서 발생하는 이벤트

      @event panelEnd
      @param {String} sType 커스텀 이벤트명
      @param {Number} no 현재 이동하는 패널의 인덱스 (0부터 시작) 
      @param {jindo.$Event} oEvent panelEnd 이벤트 객체
  **/
	_firePanelEndEvent : function() {
		this.fireEvent("panelEnd", this._htParam);
		this._htParam = {};
	},
...
jindo.m.Cover = jindo.$Class({
	...
	set : function(htElement) {
		...
		htCss[this.prefix("TransitionProperty")] = "-" + this._sCssPrefix + "-transform";
		htCss[this.prefix("Transform")] = "translate" + this._htTans.open + "0,0" + this._htTans.end;
		if(this.option("bUseTimingFunction")) {
    	htCss[this.prefix("TransitionTimingFunction")] = "cubic-bezier(0.33,0.66,0.66,1)";
    }
    // 각각 패널들 설정
		jindo.$A(aP).forEach(function(v,i,a) {
			v.css(htCss);
		});
		this.fireEvent("set", {
    	elements : htElement,
	...
	// cover는 resize의 의미가 없음. 모두 100%
	resize : function(htElement) {
		var ht = jindo.m.Animation.prototype.resize.call(this, htElement);
		htElement["container"].css(ht.property, "100%");
		return ht;
	},

	move : function(nX, nY, option) {
		var nRange = this.getRange();
		if(this._bUseH) {
			nX = this._getPos(nX, nRange, option);
		} else {
			nY = this._getPos(nY, nRange, option);
		}
		this.getTarget(true).css(this.prefix("Transform"), "translate" + this._htTans.open + nX + "px," + nY + "px" + this._htTans.end);	
		this._zoom(this._bUseH ? nX : nY, nRange, option.next);
	},
	 ...
	 */
	changeTarget : function(welTarget) {
		if(this._welTarget && this._welTarget.isEqual(welTarget)) {
			return;
		}
		// 기존 엘리먼트 복원
		if(this._welTarget) {
			this._clearElement(this._welTarget, welTarget);
		}
		// Target 변환
		var htCss = {
			"zIndex" : 10,
		...
		htCss[this.prefix('Transform')] = "translate" + this._htTans.open + "0px,0px" + this._htTans.end;
		// console.debug("Target:",welTarget.attr("id"));
	 ...
	 * @param  {$Element} welZoom   zoom-in 될 엘리먼트
	 */
	changeZoom : function(welZoom, bNext) {
		if(this._welZoom && this._welZoom.isEqual(welZoom)) {
			return;
		}
		// 기존 엘리먼트 복원
		if(this._welZoom) {
			this._clearElement(this._welZoom, welZoom);
		}
		if(welZoom) {
			var htCss = {
				"zIndex" : 1,
			...
			htCss[this.prefix('Transform')] = "translate" + this._htTans.open + "0px,0px" + this._htTans.end + " scale" + this._htTans.open + this.option("nDefaultScale") + "," + this.option("nDefaultScale") + this._htTans.open;
			// console.debug("Zoom:", welZoom.attr("id"), bNext, this._welZoom);
	 ...
	 * 타겟이 변경될때 초기화 한다.
	 */
	_clearElement : function(wel ,AfterWel) {
		var htCss = {};
		htCss[this._sCssPrefix +'Transform'] = "";
		htCss[this._sCssPrefix +'TransitionDuration'] = "";
		htCss[this._sCssPrefix +'TransitionProperty'] = "";
		htCss["display"] = "none";
		wel.css(htCss);
	},

	_getPos : function(nPos, nRange, option) {
		var n=nPos,
			bNext = option.next;
		if(option.restore) {
			n = 0;
		...
		// console.warn(nPos + " => " + n,  option, "range/"+ nRange);
		return n;
	},

	
	/**
	 * duration이 지정될때 발생함. (상위 엘리먼트에서 duration 설정될 경우 호출됨)
	 * @param {Number} nDuration 이동시간
	 */
	setDuration : function(nDuration) {
		this.getTarget(true).css(this.prefix("TransitionDuration"), nDuration);
		if(this._welZoom) {
			this._welZoom.css(this.prefix("TransitionDuration"), nDuration);
		}
	},
	 ...
	 * @param  {Boolen} bNext 다음으로 이동하는 경우 true, 이전으로 이동하는 경우 false
	 */
	_zoom : function(nPos, nRange, bNext) {
		var nOpacity = bNext ?  (nRange- nPos) / nRange -1 : 1 - (nRange - nPos) / nRange,
			nScale = this.option("nDefaultScale"),
			htCss = {};
		if(this._welZoom) {
			nScale = nScale + nOpacity * (1 - nScale);
			htCss["opacity"] = nOpacity;
			htCss[this.prefix('Transform')] = "translate" + this._htTans.open + "0px,0px" + this._htTans.end + " scale" + this._htTans.open + nScale + ", " + nScale + this._htTans.end;
			this._welZoom.css(htCss);
		}
	}
...
jindo.m.Cube = jindo.$Class({
    ...
    _setHalfWidth : function(htElement){
	...
	set : function( htElement ){
		...
		this._setHalfWidth(htElement);
		// this._nHalfWidth = parseInt(htElement["view"].css(this._bUseH  ? "width" : "height"), 10)/2;
		htCss[this.prefix("TransitionProperty")] = "-" + this._sCssPrefix + "-transform";
		htCss[this.prefix("Transform")] = "translateZ(-"+this._nHalfWidth+"px) rotate"+this._sRotate+"("+this._nDefaultDeg+"deg)";
        ...
        htCss[this.prefix("TransformStyle")] = "preserve-3d";
        htElement["view"].css(this.prefix("Perspective") , this._nHalfWidth*4+"px");        // perspective 의 값인 컨텐츠 넓이의 두배를 적용하는것이 자연스러움.
		this._welTarget = htElement["container"].css(htCss);
		this.setPanelPos(htElement, this.option("nDefaultIndex"));
     ...
     * 
     * @param {HashTable} htElement CubeFlicking 으로 부터 전달받은 element 변수 
     * @param {Number} nIndex   가운데 노출한 index 정보 
     */
	setPanelPos : function( htElement , nIndex ){
	    this._setHalfWidth(htElement);
	    nIndex = nIndex || 0;
	   var nWidth = this._nHalfWidth,
            htCss = null,
            self = this;
        ...
        function setHtCss(nNum){
            ...
            htCss["float"] = "left";
            htCss[self.prefix("BackfaceVisibility")] = "hidden";
            var nDeg = self._nDefaultDeg;
            ...
            if( !self._bUseCircular && ( nNum < (nIndex - 1) || nNum > (nIndex + 1) )){
                ...
                htCss["display"] = "block";
                htCss[self.prefix("Transform")] = "rotate"+self._sRotate+"("+ nDeg +"deg) translateZ("+ nWidth +"px) ";    
            }
        }

        jindo.$A(htElement["aPanel"]).forEach(function(v,i,a){
            setHtCss(i);
            v.css(htCss);
        });  
	},
	...
	resize : function(htElement) {
		...
		if(this._bUseCircular) {
			htContCss[ht.property] = ht.viewSize + "px";
			htCss[ht.property] = ht.viewSize + "px";
		} else {
			htCss[ht.property] = ht.viewSize + "px";
			htContCss[ht.property] = ht.viewSize + "px";
			// htContCss[ht.property] = (ht.panelCount * ht.viewSize) + "px";
		}

		// css 적용
		htElement["container"].css(htContCss);
		jindo.$A(htElement["aPanel"]).forEach(function(v,i,a){
   		   v.css(htCss);
		});
		return ht;
    ...
    /**
     *  transition duration 지정
     * @param {Number} nDuration    Duration value
     */
	setDuration : function(nDuration) {
		this.getTarget(true).css(this.prefix("TransitionDuration"), nDuration);
	},
     ...
     */
	move : function(nX, nY, option) {
	    var nPos = this._bUseH ? nX : -nY;
	    if(!option){
	        option = {};
	    }
	    nPos = this._nDefaultDeg + ((nPos * 45) / this._nHalfWidth);
		
		var welTarget = this.getTarget(true);
		welTarget.css(this.prefix("Transform"), "translateZ(-"+ this._nHalfWidth +"px) rotate"+this._sRotate+"(" + nPos + "deg)");
		
	    // var n = (nPos / this.getRange() % 4);
	    // if(this.option("bUseMomentum"))
	       // momentun 인경우와 duration 이 존재할경우 위치에 대한 오류로 대응.
	    // else{
    	    // if(option.sType && option.no > 0){
    	        // if(option.next || (!option.restore && option.next)){
    	            // if(parseInt(n) == 0){
    	                // n =4 * (this._bUseH ? -1 : 1);
    	            // }
    	            // console.log(n);
...
//     	            
    ...
    /**
     * 패널 위치 재정의 함수 
     * @param {HashTable} htInfo   
     *      htInfo.sPosition : "left",        // vertical ("top"), Horizon ("left");
     *      htInfo.nPrev : 0,                   // 이전 패널의 index 정보 
     *      htInfo.nNext : 2,                   // 다음 패널의 index 정보 
     *      htInfo.nCenter : 1,                // 보여지고 있는 패널의 index 정보 
     *      htInfo.bNext : true                // 이전 패널의 방향성이 아닌 다음 방향성으로 이동했을때 true , 반대 false
     */
    restorePanel : function(htInfo){
        var sTransform = this.prefix("Transform");
        var sRotate = "rotate"+this._sRotate;
        var nDeg = 90 * (this._bUseH ? 1 : -1);
        var nHalfWidth = this._nHalfWidth;
        
        function fnSetPanelPos(index, deg){
            htInfo.htElement["aPanel"][index].css(sTransform, sRotate + "(" + deg + "deg) translateZ(" + nHalfWidth + "px)");
        }
        if(this._bUseCircular) {
            //htInfo.bNext == undefined 인 경우 --> refresh 함수 호출을 통한 접근시..
            if(htInfo.nContentIndex == 0 || typeof htInfo.bNext == "undefined" || htInfo.nContentIndex == this.option("nTotalContents") - 1){
                // 위치 재 정의시 뒤로 이동하는 모션이 발생하여 이를 처리하기 위한 수정 
                htInfo.htElement["container"].css(this.prefix("TransitionDuration"),"")
                                                             .css(sTransform,"translateZ(-"+ this._nHalfWidth +"px) "+sRotate+"("+(htInfo.nContentIndex % 4) * -nDeg+"deg) ");
                fnSetPanelPos( htInfo.nPrev , (htInfo.nContentIndex - 1) * nDeg );
                fnSetPanelPos( htInfo.nCenter , (htInfo.nContentIndex) * nDeg );
                fnSetPanelPos( htInfo.nNext , (htInfo.nContentIndex + 1) * nDeg );
...
// 	    
        ...
        // if(this._bUseCircular) {
            ...
            // // } else 
                // console.log(n);
            // if(nPos > 0 && !bNext && this._bUseH){
                // n = ((this.option("nTotalContents") + 1) * -90) + n;
            // }else if (!bNext){
                // n -= (4 - (this.option("nTotalContents") % 4)) * 90;
            // }
                // // n = (this.option("nTotalContents") % 4) * 90 + n;
                // // n = (this.option("nTotalContents") % 4) * 90 + n;
        // }
        // return n;
	// }
...
jindo.m.Slide = jindo.$Class({
    ...
    if(this._bUseCircular) {
    	htCss[this._bUseH ? "left" : "top"] = "-100%";
    }
		this._welTarget = htElement["container"].css(htCss);
		this._setPanel(htElement, htCloneCss);
    this.fireEvent("set", {
    	elements : htElement,
    	css : htCloneCss
    });
	...
	// 패널처리
	_setPanel : function(htElement, htCss) {
		if(htElement["aPanel"]) {
			if(this._bUseCircular) {
				htCss["position"] = "absolute";
			...
			jindo.$A(htElement["aPanel"]).forEach(function(v,i,a){
	   		if(this._bUseCircular) {
	   			htCss[this._bUseH ? "left" : "top"] = ((100 * (i+1))%300) + "%";
	   		}
	   		v.css(htCss);
			},this);
		}
	...
	move : function(nX, nY, option) {
		...
		if(this._bUseCircular) {
			...
			welTarget.css(this.prefix("Transform"), "translate" + this._htTans.open + nX + "%," + nY + "%" + this._htTans.end);
		} else {
			...
			welTarget.css(this.prefix("Transform"), "translate" + this._htTans.open + nX + "px," + nY + "px" + this._htTans.end);
		}
	},

	resetTarget : function() {
		var htCss = {};
		htCss[this.prefix("TransitionDuration")] = "";
		htCss[this.prefix("Transform")] = "translate" + this._htTans.open + "0px,0px" + this._htTans.end;
		this.getTarget(true).css(htCss);
	},
	...
	_getPos : function(nPos, nRange, option) {
		...
		if(option.restore) {
			...
			if(nPos%nRange === 0) {
				...
				n = ((n/nRange*100) % 100) + (bNext ? 0 : 200);
				n %= 100;
			}
		}
		// console.warn(nPos + " => " + n,  option, "range/"+ nRange);
...
jindo.m.SwapCommon = jindo.$Class({
  ...
  **/
  ...
  // Animation 구현 이벤트 (하위 클래스에서 구현)
  // Animation의 start 이벤트 핸들러
  _onStartAniImpl : function(we) {},
  // Animation의 panelStart 이벤트 핸들러
  ...
  // Animation의 panelEnd 이벤트 핸들러
  _onPanelEndAniImpl : function(we) {},
  // Animation의 end 이벤트 핸들러
  _onEndAniImpl : function(we) {},

	set : function(oAni) {
		this._oAnimation = oAni;
		this._oAnimation.set(this._htWElement);
  ...
  **/
  ...
  _startImpl : function(we) {
    ...
    this._clearAnchor();
    this._oAnimation.clearDuration();
  },
  ...
  **/
  ...
  _getAnimationOption : function() {
    var ht = {
      sTargetValue : this._bUseH ? "x" : "y",
      bHasOffsetBug : this._hasOffsetBug()
    };
    if(typeof this.option("bUseCircular") !== "undefined") {
      ht["bUseCircular"] = this.option("bUseCircular");
    }
    if(typeof this.option("bUseCss3d") !== "undefined") {
      ht["bUseCss3d"] = this.option("bUseCss3d");
    }
    if(typeof this.option("bUseTimingFunction") !== "undefined") {
      ht["bUseTimingFunction"] = this.option("bUseTimingFunction");
    }
    return ht;
  },
  ...
  **/
   ...
   */
	_refreshPanelInfo : function(){
	  var nTotal = 0,
	  	nRange = this._oAnimation.getRange();
	...
	isAnimating : function() {
		if(this._oAnimation) {
			return this._oAnimation.isMoving();
		}
		return false;
	},
	...
	/**
	 * Animation를 등록한다.
	 * @param {jindo.m.Animation} oAni Animation를 등록한다.
	 *
	 * @method set
	 */
	set : function(oAni) {
		jindo.m.SwapCommon.prototype.set.call(this, oAni);
  ...
  resize : function() {
  	...
  	this._oAnimation.setRange(this._bUseH ? this._htSize.viewWidth : this._htSize.viewHeight);
  	this._refreshPanelInfo();
    
    // 애니메이션 영역 resize
    this._oAnimation.resize(this._htWElement);
  },
   ...
   */
	...
	/**
	 * 특정 패널로 이동한다.
	 * @param  {Number} nIndex     이동하려는 판 인덱스 (0부터)
	 * @param  {Number} nDuration  이동하는 시간 (기본은 nDuration 옵션값)
	 * @param  {Boolean} bRearrange 재정렬여부 (기본은 false)<br/> 이값이 true인 경우에는 beforeFlicking, flicking이벤트가 발생하며, bCorrupt값이 true로 반환된다.
	 *
	 * @method moveTo
	 */
	moveTo : function(nIndex, nDuration, bRearrange){
		var nMax = this.getTotalContents();
	  if(this._oAnimation.isMoving() || isNaN(nIndex) || nIndex < 0 || nIndex >= nMax) {
	  	return;
	  }
	  nDuration = (typeof nDuration === 'undefined' ? this.option('nDuration') : nDuration);
    bRearrange = (typeof bRearrange === 'undefined' ? false : true);
	  ...
	  if(nStart == nEnd) {
		...
		if(this._bUseCircular) {
			if(nStart >= 0 && nIndex == nMax-1) {
				nEnd= this.getAnimation().getRange();
			} else if(nStart <= (this._bUseH ? this._htSize.maxX : this._htSize.maxY) && nIndex == 0) {
				nEnd= this._aPos[nMax];
			}
		}	
	  this._oAnimation.run(this._nX, this._nY, this._bUseH ? nEnd : 0, this._bUseV ? nEnd : 0, nDuration, {
				direct : nDuration === 0, 	// duration이 0일 경우. 바로 이동
				rearrange : bRearrange
		});
  },
  ...
  _endImpl : function(we) {
    var ht = this._getMomentumData(we,1.5),
    ...
    if(isRestore) {
    	...
    	if(ht.duration === 0 || 
    		...
    		var nMaxPos = this.getAnimation().getRange()-1;
    		if(nDis > nMaxPos) {
    			if(this._bUseH) {
  ...
  // rotate될 때
	_resizeImpl : function(we) {},	// 하위에서 구현

  // 포지션값을 인덱스로 변경
  _posToIndex : function(nPos) {
  	return Math.abs(Math.floor(nPos/this.getAnimation().getRange()));
  },
  ...
  _restore : function() {
    ...
    var nPos = this._aPos[this.getContentIndex()],
    	nNewX = this._bUseH ? nPos : 0,
      nNewY = this._bUseV ? nPos : 0;
    if (nNewX === this._nX && nNewY === this._nY) {
        /* 최종 종료 시점 */
      return;
    } else {
			this._oAnimation.run(this._nX, this._nY, nNewX, nNewY, this.option("nBounceDuration"), {
				restore : true
			});
    }
  },
  ...
  // panelStart 이벤트
	_onPanelStartAniImpl : function(we) {
		if(we.restore) {
			...
			if(!this._fireRestoreEvent('beforeRestore',we)) {
				we.stop();
			}
		} else {
			// console.warn("  . [panelStart] - beforeFlicking ", this.getContentIndex());
			if(!this._fireFlickingEvent('beforeFlicking', we)) {
				// stop이벤트 호출시 복원하는 기능
				we.stop();
				var self = this;
				setTimeout(function() {
  ...
  _onPanelEndAniImpl : function(we) {
  	if(we.restore) {
  		// console.warn("  . [panelEnd] - restore ", we.no);
   		this._fireRestoreEvent('restore', we);
   	} else {
	   	// 순환인 경우, index를 보정함
	   	var no = we.no,
	   	...
	   	we.no = no;
	   	this._updateFlickInfo(no, we.next ? this.getNextElement() : this.getPrevElement());
			/**
      ...
      **/      
   		...
   		this._fireFlickingEvent('flicking', we);
   	}
  },
  ...
  _fireFlickingEvent : function(name, we) {
    var	nMoveIdx = we.next ? this.getNextIndex() : this.getPrevIndex(),
		...
		// beforeFlicking일 경우, moveCount가 1보다 클경우, 다음 인덱스 값을 보정
		if(name == "beforeFlicking" && we.moveCount > 1) {
			ht.nContentsNextIndex = ht.nContentsIndex + (ht.bNext ? we.moveCount : -we.moveCount);
		}
		// we에 동일 항목이 존재할 경우
  ...
  _updateFlickInfo : function(nIndex, wel) {
  	...
  	if(typeof wel === "undefined") {
		...
		var nRange = this.getAnimation().getRange();
		// 좌표 조정
  	this._nX = this._bUseH ? nIndex * -nRange : 0;
		this._nY = this._bUseV ? nIndex * -nRange : 0;
		// 엘리먼트 조정
...
jindo.m.CoverFlicking = jindo.$Class({
    ...
    @param {Object} [htOption] 초기화 옵션 객체
      ...
      @param {Number} [htOption.nDefaultIndex=0] 초기 로드시의 화면에 보이는 콘텐츠의 인덱스
			...
			@param {Number} [htOption.nDefaultScale=0.94] cover될때 줌인되는 패널의 기본 크기
  ...
  **/
	...
	// _setWrapperElement : function(el) {
	// 	jindo.m.Flick.prototype._setWrapperElement.call(this,el);
	// },
	...
	_changeTarget : function(bNext) {
		...
		this._oAnimation.changeZoom(welZoom, bNext);
		this._oAnimation.changeTarget(welTarget);
	},
	...
	moveTo : function(nIndex, nDuration, bRearrange) {
		...
		jindo.m.Flick.prototype.moveTo.call(this, nIndex, nDuration, bRearrange);
	},
	 ...
	 */
  _onPanelEndAniImpl : function(we) {
  	jindo.m.Flick.prototype._onPanelEndAniImpl.call(this,we);
...
jindo.m.CubeFlicking = jindo.$Class({
    ...
    **/
    ...
    _onActivate : function() {
        jindo.m.Flick.prototype._onActivate.apply(this);
        this.set(new jindo.m.Cube({
            bUseCircular : this.option("bUseCircular"),
            sTargetValue : this._bUseH ? "x" : "y",
    ...
    _restorePanel : function(wel, bNext) {
        ...
        var htInfo = {
            ...
            "htElement" : this._htWElement
        };
        
        this._oAnimation.restorePanel(htInfo);
        
        // console.log(nPrev + "|" + nCenter + "|" + nNext);
    },
    ...
    _onPanelEndAniImpl : function(we) {
        jindo.m.Flick.prototype._onPanelEndAniImpl.call(this,we);
        if(this._bUseCircular) {
            this._restorePanel("", we.next);
        }else{
            this._oAnimation.setPanelPos( this._htWElement, we.no);
        }
    },
...
jindo.m.SlideFlicking = jindo.$Class({
  ...
  **/
  ...
  _resizeImpl : function(we) {
	...
	_restorePanel : function(wel) {
		...
		this._welElement = this._htWElement["aPanel"][nCenter]
			.css(sPosition, "100%").css('zIndex',10);
		this._htWElement["aPanel"][nPrev].css(sPosition, "0%").css('zIndex',1);
		this._htWElement["aPanel"][nNext].css(sPosition, "200%").css('zIndex',1);
	},
  ...
  // _onEndAniImpl : function(we) {
  // 	jindo.m.Flick.prototype._onEndAniImpl.call(this, we);
  // },
...
jindo.m.SlideReveal = jindo.$Class({
    ...
    **/
    ...
    _attachEvent : function(){
        ...
        // this._htEvent["resize"] = jindo.$Fn(this._resize, this).attach(window "resize");
    },
...
//                 
    ...
    _onResize : function(){
        ...
        **/
        ...
        var htPosInfo = this._getPosInfo();
        this._bRotate = true;
        if(htPosInfo.nMarginLeftPos > 0  && this.option("sDirection") == "left"){
            if(!this._bShow){
                return false;
            }
            if(!this.option("bUseOffsetBug")){
                            
                this._htTransition["header"].queue(this._htWElement["header"].$value(), 0, {
                    htTransform : {
                        "transform" : this._sTranslateStart + -htPosInfo.nMarginLeftPos+"px, "+htPosInfo.nHeader.Y+"px" + this._sTranslateEnd
                    }
                });
                this._htTransition["content"].queue(this._htWElement["content"].$value(), 0, {
                    htTransform : {
                ...
                this._htTransition["nav"].queue(this._htWElement["nav"].$value(), 0, {
                    htTransform : {
                        "transform" : this._sTranslateStart + (-this._nWidth + this.option("nMargin"))+"px, "+htPosInfo.nNav.Y+"px" + this._sTranslateEnd
                    }
                });
    
                this._htTransition["nav"].start();
                this._htTransition["header"].start();
                this._htTransition["content"].start();
            }else{
                this._htWElement["nav"].css("left", this.option("nMargin"));
    ...
    /**
     *  내부 css 변경 공통 함수
     *  -webkit-transform 등 변경.
     */
    // _setCss : function(welEl, sAttr, sValue){
        // welEl.css(this._sCssPrefix + sAttr, sValue);
    // },
    ...
    _createTransition : function(){
        ...
        var self = this;
        this._htTransition["nav"] = new jindo.m.Transition(htInOption).attach({
            "end" : function(){
                self._setNavPos();
            }
        });
        this._htTransition["header"] = new jindo.m.Transition(htInOption).attach({
            "end" : function(){
                self._setHeaderPos();
            }
        });
        this._htTransition["content"] = new jindo.m.Transition(htInOption).attach({
            "end" : function(){
                self._setContentPos();
            }
        });
    ...
    _setMoveCss : function(bType, nDuration){
       ...
       // if(bType){
            // this._bShow = bType == "show" ? true : false;
       // }
        ...
        if(!this._bNavInHeader){
            ...
            this._htTransition["nav"].queue(this._htWElement["nav"].$value(), nDuration, {
                htTransform : {
        ...
        this._htTransition["content"].queue(this._htWElement["content"].$value(), nDuration, {
            htTransform : {
        ...
        var self = this;
        setTimeout(function(){
            self._htTransition["header"].start();
            self._htTransition["nav"].start();
            self._htTransition["content"].start();
            // self._clearCssAttr(bType);
        }, 1);
    ...
    _setContentPos : function(){
        if(this.option("bUseOffsetBug")){
            ...
            this._htTransition["content"].stop();
            this._htWElement["content"].css({
    ...
    _setHeaderPos : function(){
        if(this.option("bUseOffsetBug")){
            ...
            this._htTransition["header"].stop();
            this._htWElement["header"].css({
                "left" : htPosInfo.nLeftPos+"px",
    ...
    _onDeactivate : function(){
        ...
        this._htWElement = this._htEvent = this._htTransition = this._bRotate = null;
    },
