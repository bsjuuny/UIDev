<template>
	...
	<fieldset>
		<legend>비밀번호 재설정 페이지</legend>
		<section class="section-reset">
			<h1 v-html="$t('resetPassword.reset-title')"></h1>
			<h2 v-html="$t('resetPassword.reset-password-title')"></h2>
			<div class="input-password">
				<h3><label for="inputPassword" v-html="$t('resetPassword.reset-password-input-title1')"></label></h3>
				<input type="password" id="inputPassword" placeholder="" v-on:blur="checkPassword" />
				<p class="error"></p>
				<h3><label for="inputCheckPassword" v-html="$t('resetPassword.reset-password-input-title2')"></label></h3>
				<input type="password" id="inputCheckPassword" placeholder="" v-on:blur="recheckPassword" />
				<p class="error"></p>
			</div>
			<div class="button-right">
				<button type="submit" class="button-check-email" v-html="$t('resetPassword.reset-password-button')"></button>
			</div>
		</section>
	</fieldset>
	</form>
	...
	export default {
		...
		methods: {
			...
			recheckPassword: function() {
				...
				if (getPwd != checkPwd) {
					objCheckPwd.next('p.error').show().text(his.$t('message.join-password-error4'));
			...
			checkSubmitForm: function() {
				if (this.checkPassword() && this.recheckPassword()) {
					...
					var settings = {
						async: true,
						crossDomain: true,
						url: "http://api-ico.kaicoin.io/mypage/resetPassword",
						...
						success : function(data, status, response) {
							if (data.isReset) {
								alert(this.$t('message.password-reset-complete'));
