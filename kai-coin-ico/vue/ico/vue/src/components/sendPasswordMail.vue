<template>
	<section class="section-join">
		<article class="mailCertification">
			...
			<h2 v-html="$t('resetPassword.reset-send-mail-title')"></h2>
			<p class="message-send-mail" v-html="$t('resetPassword.reset-send-mail-content')"></p>
		</article>
		<div class="button-right">
			<button type="button" class="resend-mail" v-on:click="reSendMail" v-html="$t('resetPassword.reset-resend-mail-button')"></button>
		</div>
	</section>
</template>
	...
	export default {
		...
		methods: {
			reSendMail: function() {
				...
				$.ajax({
					url:"http://api-ico.kaicoin.io/mypage/sendPasswordResetMail/"+getEmail+"/",
					type:"get",
