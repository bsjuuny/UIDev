		...
		<section class="section-join">
			...
			<div class="join-name">
				<div class="left">
					...
					<input type="text" id="joinName" placeholder="" v-on:blur="checkName" />
					<p class="error"></p>
				</div>
				<div class="right">
					<h2><label for="joinFamilyName" v-html="$t('join.join-family-name')"></label></h2>
					<input type="text" id="joinFamilyName" placeholder="" v-on:blur="checkFamilyName" />
					<p class="error"></p>
				</div>
			</div>
			...
			<p class="agree-policy">
				...
				<label for="agree" v-html="$t('join.join-terms')"></label>
			</p>
		...
		methods: {
			checkEmail: function() {
				...
				var settings = {
					async: false,
					crossDomain: true,
					url: "http://api-ico.kaicoin.io/auth/validateEmail/"+getEmail+"/",
					method: "GET",
					headers: {},
					beforeSend: function() {
			...
			checkSubmitForm: function() {
				if (this.checkEmail() && this.checkName() && this.checkFamilyName() && this.checkPassword() && this.recheckPassword() && this.checkTerm()) {
					...
					$.ajax({
						url:"http://api-ico.kaicoin.io/auth/signup",
						type:"post",
