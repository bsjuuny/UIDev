		...
		methods: {
			checkState: function() {
				var settings = {
					async: true,
					crossDomain: true,
					url: "http://api-ico.kaicoin.io/mypage/validateEmailForPasswordReset/"+this.getEmail+"/",
					method: "GET",
					headers: {},
					beforeSend: function() {
					...
					error : function(request, status, error) {
						...
						alert(this.$t('resetPassword.reset-password-mail-error'));
						$.unblockUI();
					}.bind(this)
				}
				$.ajax(settings);
			...
			checkSubmitForm: function() {
				...
				var settings = {
					async: true,
					crossDomain: true,
					url: "http://api-ico.kaicoin.io/mypage/sendPasswordResetMail/"+getEmail1+"/",
					method: "GET",
					headers: {},
					beforeSend: function() {
						$.blockUI({message: $('#loader'), centerY: true, css: {left:'50%', width:'200px', border:0, margin:'0 0 0 -100px'}});
