					...
					success : function(data, status, response) {
						if (data) {
							...
							switch (data.state) {
								case 'R': 
									$.ajax({
										url:"http://api.kaicoin.co.kr/auth/sendSignupMail/"+email+"/",
										type:"get",
									...
									return false;
								case 'E': alert(this.$t('message.move-certify'));this.$router.push('/mailConfirm/'+data.state+'/'+email); return false;
								case 'M': this.$router.push('/dashboard'); return false;
								default: this.$router.push('/login'); return false;
							}
						}
					}.bind(this),
					error : function(request, status, error) {
